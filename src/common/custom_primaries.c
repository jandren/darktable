...
static inline float determinant(const float a, const float b, const float c, const float d)
{
  return a * d - b * c;
}

static inline float intersect_line_segments(const float x1, const float y1, const float x2, const float y2,
...
static inline float find_distance_to_edge(const dt_iop_order_iccprofile_info_t * const profile,
...
void dt_make_custom_primaries(const dt_iop_order_iccprofile_info_t * const profile,
                              const float scaling[3], const float rotation[3],
                              float custom_primaries[3][2])
{
  ...
  for(size_t i = 0; i < 3; i++)
  {
    const float dx = profile->primaries[i][0] - profile->whitepoint[0];
    const float dy = profile->primaries[i][1] - profile->whitepoint[1];
    const float angle = atan2f(dy, dx) + rotation[i];
    const float cos_angle = cosf(angle);
    const float sin_angle = sinf(angle);
    const float distance_to_edge = find_distance_to_edge(profile, cos_angle, sin_angle);
    const float dx_new = scaling[i] * distance_to_edge * cos_angle;
    const float dy_new = scaling[i] * distance_to_edge * sin_angle;
    custom_primaries[i][0] = dx_new + profile->whitepoint[0];
    custom_primaries[i][1] = dy_new + profile->whitepoint[1];
  }
}
